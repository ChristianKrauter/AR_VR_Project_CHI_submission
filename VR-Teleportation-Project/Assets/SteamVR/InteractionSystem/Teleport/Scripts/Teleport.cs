...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
		...
		private void UpdatePointer()
		{
            ...
            Destroy(GameObject.FindObjectOfType<LineRenderer>().gameObject);
            actualLineRenderer = actualNewObject.AddComponent<LineRenderer>();

            RaycastHit hitInfo2;

            if (Physics.Linecast(pointerStart, pointerStart+pointerDir*100, out hitInfo2, traceLayerMask))
            {
                ...
                this.actualLineRenderer.material = pointVisibleMaterial;
                this.actualLineRenderer.startWidth = .5f;
                this.actualLineRenderer.endWidth = .5f;
                this.actualLineRenderer.SetPosition(0, pointerStart);
                this.actualLineRenderer.SetPosition(1, hitInfo2.transform.position);
            }
			...
			{
                // CHRISCHANGED true -> false
                pointerAtBadAngle = false;
			}
		...
		{
            //POSSIBLE VISUALIZATION HERE -- JONAS
			teleporting = true;
		...
		private void TeleportPlayer()
		{
			...
			if ( teleportArea != null )
			{
                //teleportArea.transform.forward;
                teleportPosition = teleportPosition + 1.5f * teleportArea.transform.forward;
                /*if ( floorFixupMaximumTraceDistance > 0.0f )
				{
					RaycastHit raycastHit;
